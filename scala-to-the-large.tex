...
\begin{frame}{Why Scala?}
	\begin{itemize}
		...
		\begin{itemize}
			\item Both in research (\bold{ScalaLoci} \href{https://scala-loci.github.io/}{\faLink}, \bold{ScaFi} \href{https://scafi.github.io/}{\faLink}) and in industry (\bold{Akka} \href{https://akka.io/}{\faLink}, \bold{Spark} \href{https://spark.apache.org/}{\faLink}, \bold{Flink} \href{https://flink.apache.org/}{\faLink}) 
		\end{itemize}
  	\item Reasons:
    \begin{enumerate}
...
threadB.start()
		...
		\begin{alertblock}{Monitor}
    		...
    		\item Implemented through \mintinline{scala}{synchronized}, \mintinline{scala}{wait}, \mintinline{scala}{notify} and \mintinline{scala}{notifyAll}
      	\item Or by using high-level API (\mintinline{scala}{java.util.concurrent})
    		\item[] \begin{tcolorbox}[left=0pt, top=0pt, bottom=0pt]
					\begin{minted}{scala}
trait Counter:
	def tick(): Unit
...
\begin{frame}[fragile, allowframebreaks]{Execution Context}
	\begin{itemize}
  	...
  	\item Similar to the \texttt{Executors} Java framework, but platform-independent (works bot from JVM, JS, Native)
   	\item Bring to pass the execution context ``implicitly'' (i.e., using \mintinline{scala}{given} \& {\color{darkgreen}\mintinline{scala}{using}}) 
		 \item[] \begin{tcolorbox}[left=0pt, top=0pt, bottom=0pt]
			\begin{minted}{scala}
...
\begin{frame}[fragile]{Parallell collections example: toward declarative parallel computations}
	\begin{itemize}
  	...
  	\item \bold{Idea}: I express a data pipeline computation and then the runtime optimizes it using several cores
		\item[] \begin{tcolorbox}[left=0pt, top=0pt, bottom=0pt]
			\begin{minted}{scala}
...
\begin{frame}[fragile]{Future}
	...
	\end{alertblock}
\begin{itemize}
 \item It is \bold{the} idiomatic way to express concurrent computations
...
with possible side effects.
    ...
    \begin{itemize}
			\item they should be used in the \emph{``end-of-the-world''} %(i.e., the main method)
		\end{itemize}
	\end{itemize}
\end{frame}
...
%The curious can check the seminar of the past year \href{https://github.com/cric96/MVC-meets-monad/releases/tag/0.1.0-2021-06-04T085146}{\faLink}
%				\end{alertblock}
\end{frame}
