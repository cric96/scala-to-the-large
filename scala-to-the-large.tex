...
threadB.start()
		...
		\begin{alertblock}{Monitor}
    		...
    		\item Implemented through \mintinline{scala}{synchronized} (not a keyword!!), \mintinline{scala}{wait}, \mintinline{scala}{notify} and \mintinline{scala}{notifyAll}
...
\begin{frame}[fragile, allowframebreaks]{Execution Context}
	\begin{itemize}
  	...
  	\item Similar to the \texttt{Executors} Java framework, but platform-independent (works bot from JVM, JS, Native)
   	\item Bring to pass the execution context ``implictily'' (i.e., using \mintinline{scala}{given} \& {\color{darkgreen}\mintinline{scala}{using}}) 
		 \item[] \begin{tcolorbox}[left=0pt, top=0pt, bottom=0pt]
			\begin{minted}{scala}
...
/* In Scala, a natural way to "enrich" 
	 the new "language" consists in using contexts + entrypoints.
*/
