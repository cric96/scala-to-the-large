...
\begin{frame}{Scala: Best choice to create complex \& distributed Framework}
	\begin{itemize}
		\item DSL hide complexity giving high-level API
  	\item Contextual information (i.e. to use API) is passed through \emph{implicit} giving the impression of "enrich" the languague
   \item Strong type system enforce compile-time correctness
   ...
   \begin{itemize}
	 ...
	 \begin{itemize}
			\item two relevant example of industry-proven scala framwork (Akka \& Spark)
   	\	\item two exaples of reasearc oriented libraries \& framework (ScaFi \% ScalaLoci) 
	 \end{itemize}
	\end{itemize}
\end{frame}
...
\end{frame}
\begin{frame}[fragile, allowframebreaks]{ScaFi (Scala Fields) -- Usage example}
